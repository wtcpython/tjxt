 ...
 */
@Service
public class CourseTeacherDraftServiceImpl extends ServiceImpl<CourseTeacherDraftMapper, CourseTeacherDraft>
        implements ICourseTeacherDraftService {
    ...
    public void save(CourseTeacherSaveDTO courseTeacherSaveDTO) {
        ...
        // 1.数据删除条件
        LambdaUpdateWrapper<CourseTeacherDraft> updateWrapper = Wrappers.lambdaUpdate(CourseTeacherDraft.class)
    ...
    public List<CourseTeacherVO> queryTeacherOfCourse(Long courseId, Boolean see) {
        if (see) {
            ...
            // 1.1.课程老师关系判非空
            if (CollectionUtils.isNotEmpty(courseTeacherVOS)) {
                return courseTeacherVOS;
            }
            // 2.查询草稿中的课程老师关系
            courseTeacherVOS = queryTeachers(courseId);
            // 3.组装数据
            return CollectionUtils.isEmpty(courseTeacherVOS) ? new ArrayList<>() : courseTeacherVOS;
        } else {
            // 4.查询草稿中的课程老师关系
    ...
    private List<CourseTeacherVO> queryTeachers(Long couserId) {
        ...
        List<CourseTeacherDraft> courseTeacherDrafts = baseMapper.selectList(queryWrapper);
        // 1.2.数据判空
        if (CollectionUtils.isEmpty(courseTeacherDrafts)) {
