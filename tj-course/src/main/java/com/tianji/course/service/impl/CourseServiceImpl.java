...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements ICourseService {
    ...
    public CourseDTO getCourseDTOById(Long id) {
        ...
        // 4.统计课程销量
        Map<Long, Integer> peoNumOfCourseMap = tradeClient.countEnrollNumOfCourse(Collections.singletonList(id));
        if (MapUtils.isNotEmpty(peoNumOfCourseMap)) {
    ...
    public List<CourseSimpleInfoDTO> getSimpleInfoList(CourseSimpleInfoListDTO courseSimpleInfoListDTO) {
        ...
        LambdaQueryWrapper<Course> queryWrapper = Wrappers.lambdaQuery(Course.class)
                .in(CollectionUtils.isNotEmpty(courseSimpleInfoListDTO.getThirdCataIds()),
                        Course::getThirdCateId, courseSimpleInfoListDTO.getThirdCataIds())
                .in(CollectionUtils.isNotEmpty(courseSimpleInfoListDTO.getIds()),
    ...
    public CourseAndSectionVO queryCourseAndCatalogById(Long courseId) {
        ...
        if (CollectionUtils.isEmpty(lessonDTO.getRecords())) {
    ...
    public List<SubNumAndCourseNumDTO> countSubjectNumAndCourseNumOfTeacher(List<Long> teacherIds) {
        ...
        Map<Long, Integer> teacherIdAndCourseNumMap = IdAndNumDTO.toMap(baseMapper.countCourseNumOfTeacher(teacherIds));
        // 1.2.待上架
        Map<Long, Integer> teacherIdAndCourseNumMap2 = IdAndNumDTO
                .toMap(courseDraftMapper.countCourseNumOfTeacher(teacherIds));
        // 1.3.统计老师的出题数量
    ...
    public Map<Long, Integer> countCourseNumOfCategory() {
        ...
        Map<Long, Integer> nomalCourseNumOfCategory = countNomalCourseNumOfCategory();
        // 2.统计课程分类待上架，已下架的课程分类的数量
        Map<Long, Integer> draftCourseNumOfCategory = courseDraftService.countCourseNumOfCategory();
        // 3.两组数据聚合
        Map<Long, Integer> result = new HashMap<>(nomalCourseNumOfCategory);
        draftCourseNumOfCategory.forEach((k, v) -> result.merge(k, v, Integer::sum));
        return result;
    }
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery) {
        ...
        Map<Long, String> updaterMap = CollectionUtils.isEmpty(updaterList) ? new HashMap<>()
                : userDTOS
                        .stream()
        ...
        Map<Long, String> categoryNameMap = CollectionUtils.isEmpty(list) ? new HashMap<>()
                : list.stream()
        ...
        return PageDTO.of(page, CoursePageVO.class, (course, coursePageVO) -> {
            // 5.1.拼接课程分类名称
            String categories = String.format("%s/%s/%s",
