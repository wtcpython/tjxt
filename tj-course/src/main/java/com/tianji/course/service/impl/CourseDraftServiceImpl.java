...
public class CourseDraftServiceImpl extends ServiceImpl<CourseDraftMapper, CourseDraft> implements ICourseDraftService {
    ...
    public CourseBaseInfoVO getCourseBaseInfo(Long id, Boolean see) {
        ...
        if (CollectionUtils.isNotEmpty(categories)) {
            ...
            // 4.2.设置课程分类名称
            courseBaseInfoVO.setCateNames(String.format("%s/%s/%s",
    ...
    public void upShelf(Long id) {
        ...
        // 4.4.5.课程发布次数
        int publishTimes = (course == null) ? 1 : NumberUtils.null2Zero(course.getPublishTimes()) + 1;
    ...
    public CourseDTO getCourseDTOById(Long id) {
        ...
        // 2.查询课程老师列表，并去序号的第一个
        LambdaQueryWrapper<CourseTeacherDraft> queryWrapper = Wrappers.lambdaQuery(CourseTeacherDraft.class)
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery) {
        // 1.课程草稿分页查询条件
        LambdaQueryWrapper<CourseDraft> queryWrapper = SqlWrapperUtils.toLambdaQueryWrapper(coursePageQuery,
                CourseDraft.class);
        // 1.1课程查询条件-更新时间
    ...
    public NameExistVO checkName(String name, Long id) {
        // 1.课程草稿同名查询条件
        LambdaQueryWrapper<CourseDraft> queryWrapper = Wrappers.lambdaQuery(CourseDraft.class)
