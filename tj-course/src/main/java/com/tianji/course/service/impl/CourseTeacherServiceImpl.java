...
public class CourseTeacherServiceImpl extends ServiceImpl<CourseTeacherMapper, CourseTeacher>
    ...
    public List<CourseTeacherVO> queryTeachers(Long couserId) {
        ...
        // 3.查询教师信息
        List<UserDTO> teacherDetailDTOS = userClient.queryUserByIds(
                courseTeachers
                        .stream()
                        .map(CourseTeacher::getTeacherId)
                        .collect(Collectors.toList()));
        // 3.1.教师id+教师信息map
        Map<Long, UserDTO> teacherDetailDTOMap = teacherDetailDTOS
                .stream()
    ...
    public List<Long> getTeacherIdOfCourse(Long courseId) {
        ...
        List<CourseTeacher> courseTeachers = baseMapper.selectList(queryWrapper);
        // 3.组装数据
        return CollectionUtils.isEmpty(courseTeachers) ? new ArrayList<>()
                : courseTeachers
                        .stream()
                        .map(CourseTeacher::getTeacherId)
                        .collect(Collectors.toList());
