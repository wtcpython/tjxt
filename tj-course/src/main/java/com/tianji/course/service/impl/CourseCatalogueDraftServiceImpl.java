...
@Service
@Slf4j
public class CourseCatalogueDraftServiceImpl extends ServiceImpl<CourseCatalogueDraftMapper, CourseCatalogueDraft>
        implements ICourseCatalogueDraftService {
    ...
    public List<CataVO> queryCourseCatalogues(Long courseId, Boolean see, Boolean withPractice) {
        if (see) {
            ...
            if (CollectionUtils.isNotEmpty(cataVOS)) {
                return cataVOS;
            }
            // 1.2查看草稿目录
            cataVOS = queryCourseCatalogues(courseId, withPractice);
            return CollectionUtils.isEmpty(cataVOS) ? new ArrayList<>() : cataVOS;
    ...
    public void saveMediaInfo(Long courseId, List<CourseMediaDTO> courseMediaDTOS) {
        ...
        List<Long> cataIds = courseMediaDTOS.stream()
    ...
    public List<CataSimpleSubjectVO> getSuject(Long courseId) {
        ...
        if (CollectionUtils.isEmpty(cataSubjectDrafts)) {
            return new ArrayList<>();
        }
        List<Long> subjectIdList = cataSubjectDrafts.stream().map(CourseCataSubjectDraft::getSubjectId)
                .collect(Collectors.toList());
    ...
    public void checkCataInfoImplated(Long courseId) {
        ...
        Map<Long, Long> subjectNumMap = CollectionUtils.isEmpty(courseCataSubjectDrafts) ? new HashMap<>()
        ...
        CollUtils.check(courseCatalogueDrafts, new Checker<CourseCatalogueDraft>() {
            ...
            public void check(CourseCatalogueDraft courseCatalogueDraft) {
                if (courseCatalogueDraft.getType() == CourseConstants.CataType.SECTION
                    ...
                    throw new BizIllegalException(String.format(
                            CourseErrorInfo.Msg.COURSE_UP_SHELF_SECTION_WITHOUT_MEDIA, courseCatalogueDraft.getName()));
                    ...
                    throw new BizIllegalException(
                            String.format(CourseErrorInfo.Msg.COURSE_UP_SHELF_PRACTICE_WITHOUT_SUBJECT,
    ...
    public void copySubjectToShelf(Long courseId, Boolean isFirstShelf) {
        ...
        // 2.删除练习和题目之间的关系
        if (CollectionUtils.isEmpty(courseCataSubjectDrafts)) {
            // 草稿中没有题目，直接结束
            return;
        }
        // 3.将新的练习和题目之间的关系上架
    ...
    public Map<Long, Integer> calculateMediaDuration(Long courseId) {
        LambdaQueryWrapper<CourseCatalogueDraft> queryWrapper = Wrappers.lambdaQuery(CourseCatalogueDraft.class)
    ...
    public List<Long> queryCataIdsOfCourse(Long courseId, List<Integer> types) {
        ...
        List<CourseCatalogueDraft> courseCatalogueDrafts = baseMapper.selectList(queryWrapper);
        // 3.返回数据
        return CollectionUtils.isEmpty(courseCatalogueDrafts)
                ? new ArrayList<>()
    ...
    private void checkIndex(List<CataSaveDTO> cataSaveDTOS, List<CourseCatalogue> courseCatalogues) {
        ...
        // 校验章序号是否有重复的
        Map<Integer, CataSaveDTO> collect = cataSaveDTOS.stream()
                .collect(Collectors.toMap(CataSaveDTO::getIndex, p -> p));
        ...
        if (CollectionUtils.isEmpty(courseCatalogues)) {
            return;
        }
        ...
        for (CataSaveDTO cataSaveDTO : cataSaveDTOS) {// 章的序号
            ...
            // 小节和练习的序号
            if (CollectionUtils.isEmpty(cataSaveDTO.getSections())) { // 章没有对应的小节
        ...
        for (CourseCatalogue courseCatalogue : courseCatalogues) {
            ...
            if (index == null) {
                throw new BizIllegalException(String.format(CourseErrorInfo.Msg.COURSE_CATAS_SAVE_CHAPTER_NAME_DELETED,
                        courseCatalogue.getName()));
            }
            // 章排序
            if (!index.equals(courseCatalogue.getCIndex())) {
                throw new BizIllegalException(String.format(ErrorInfo.Msg.OPERATE_FAILED, courseCatalogue.getName()));
            }
        }
    }
     ...
     */
    private List<CourseCatalogueDraft> packageCatalogue(Long courseId, List<CataSaveDTO> cataSaveDTOS,
            List<CourseCatalogue> shelfCourseCatalogues) {
        final Map<Long, CourseCatalogueDraft> savedMap = new HashMap<>();
        if (CollectionUtils.isNotEmpty(shelfCourseCatalogues)) {
            // 将已经上架的目录copy下来
            shelfCourseCatalogues.stream().forEach(courseCatalogue -> savedMap.put(courseCatalogue.getId(),
                    BeanUtils.toBean(courseCatalogue, CourseCatalogueDraft.class)));
        }
        // 获取已经保存的草稿
        LambdaQueryWrapper<CourseCatalogueDraft> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(CourseCatalogueDraft::getCourseId, courseId);
        List<CourseCatalogueDraft> savedCourseCataloguesDraft = list(queryWrapper);
        if (CollectionUtils.isNotEmpty(savedCourseCataloguesDraft)) {
            // 存在草稿覆盖已经上架的课程
            savedCourseCataloguesDraft.stream()
                    .forEach(courseCatalogueDraft -> savedMap.put(courseCatalogueDraft.getId(), courseCatalogueDraft));
        }
        // 需要保存的草稿
        ...
        for (CataSaveDTO cataSaveDTO : cataSaveDTOS) {
            ...
            // 设置目录添加或修改时的基本信息
            courseCatalogueDraft.setCataBaseInfo(cataSaveDTO.getIndex(), cataSaveDTO.getName(),
                    CourseConstants.CataType.CHAPTER,
                    0L, courseId);
            ...
            cataSaveDTO.getSections().stream().forEach(section -> {
                ...
                Integer index = section.getType() == CourseConstants.CataType.SECTION ? indexCount.incrementAndGet()
                        : null;
    ...
    private List<CataVO> queryCourseCatalogues(Long courseId, Boolean withPractice) {
        ...
        List<CourseCatalogueDraft> courseCatalogueDrafts = baseMapper.selectList(queryWrapper);
        if (CollectionUtils.isEmpty(courseCatalogueDrafts)) {
            return null;
        }
        CourseDraft courseDraft = courseDraftService.getById(courseId);

        // 最大上架数,待上架设置空map，已上架需要排序并去小节序号（同一个章中）中最大小节
        Map<Long, CourseCatalogueDraft> chapterIdAndMaxSectionMap = (courseDraft.getStatus() == CourseStatus.NO_UP_SHELF
                .getStatus())
                        ? new HashMap<>()
                        : courseCatalogueDrafts.parallelStream()
                                ...
                                .collect(Collectors.groupingBy(CourseCatalogueDraft::getParentCatalogueId,
                                        Collectors.collectingAndThen(
                                                Collectors.reducing(
                                                        (c1, c2) -> c2.getCIndex().compareTo(c1.getCIndex()) > 0 ? c2
                                                                : c1),
                                                Optional::get)));
        ...
        // 4.1.统计题目数量
        Map<Long, Long> cataIdAndNumMap = CollectionUtils.isEmpty(subjects) ? new HashMap<>()
                : subjects.stream()
                        .collect(Collectors.groupingBy(CourseCataSubjectDraft::getCataId, Collectors.counting()));
        // 4.2.查询分数
        Map<Long, Integer> cataIdAndTotalScoreMap = new HashMap<>(cataIdAndNumMap.size());
        if (CollectionUtils.isNotEmpty(subjects)) {
            Set<Long> questionIds = subjects.stream().map(CourseCataSubjectDraft::getSubjectId)
        ...
        return TreeDataUtils.parseToTree(courseCatalogueDrafts, CataVO.class, (catalogueDraft, vo) -> {
            ...
            if (catalogueDraft.getType() == CourseConstants.CataType.SECTION) {
                // 小节最大编辑数
                CourseCatalogueDraft courseCatalogueDraft = chapterIdAndMaxSectionMap
                        .get(catalogueDraft.getParentCatalogueId());
    ...
    private void checkPracticeIds(List<Long> cataIds, Long courseId) {
        ...
        if (!CollectionUtils.containsAll(allCataIdList, cataIds)) {
        ...
        // 所有练习的id列表
        List<Long> practiceIdList = courseCatalogueDrafts.stream()
                .filter(practice -> practice.getType() == CourseConstants.CataType.PRATICE)
                .map(CourseCatalogueDraft::getId).collect(Collectors.toList());
        // 判断前端传过来的章节id列表是否符合是课程练习id父集合
        if (!CollectionUtils.containsAll(cataIds, practiceIdList)) {
    ...
    private void checkSectionIds(List<Long> cataIds, Long courseId) {
        ...
        if (CollectionUtils.size(courseCatalogueDrafts) != CollectionUtils.size(cataIds)) {
            throw new BizIllegalException(CourseErrorInfo.Msg.COURSE_MEDIA_SAVE_MEDIA_NULL);
        }
        // 4.转化出数据库中课程小节id列表
        List<Long> cataIdsInDb = courseCatalogueDrafts
                .stream()
                .map(CourseCatalogueDraft::getId)
                .collect(Collectors.toList());
        // 5.取前端传来小节id列表和数据库中小节id列表交集
        Collection<Long> cataIdsOfIntersection = CollectionUtils.intersection(cataIds, cataIdsInDb);
    ...
    private List<CourseCatalogueDraft> calculateCatalogMediaDuration(Long courseId) {
        ...
        // 3.统计每个章的课时总时长
        Map<Long, Integer> capthIdAndMediaDurationMap = courseCatalogueDrafts.stream().collect(Collectors.groupingBy(
                CourseCatalogueDraft::getParentCatalogueId,
                Collectors.summingInt(CourseCatalogueDraft::getMediaDuration)));
        // 4.封装数据
