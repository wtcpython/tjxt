...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements ICategoryService {
    ...
    public void delete(Long id) {
        ...
        // 1.2.子分类判空
        if (CollectionUtils.isNotEmpty(categories)) { // 分类有子分类
    ...
    public void disableOrEnable(CategoryDisableOrEnableDTO categoryDisableOrEnableDTO) {
        ...
        if (CollectionUtils.isNotEmpty(categories)) { // 直接子分类
        ...
        if (CollectionUtils.isNotEmpty(childCategoryIds)) {
            ...
            if (CollectionUtils.isNotEmpty(inDirectCategorys)) {
    ...
    public List<SimpleCategoryVO> all(Boolean admin) {
        ...
        // 1.1.判空
        if (!admin && CollectionUtils.isEmpty(categoryIdList)) {
            return new ArrayList<>();
        }
        ...
        LambdaQueryWrapper<Category> queryWrapper = Wrappers.lambdaQuery(Category.class)
                ...
                .in(CollectionUtils.isNotEmpty(categoryIdList), Category::getId, categoryIdList)
                .orderByAsc(Category::getPriority)
    ...
    public Map<Long, String> queryByThirdCateIds(List<Long> thirdCateIdList) {
        ...
        // 1.校验
        // 1.1判断参数是否为空
        if (CollectionUtils.isEmpty(thirdCateIdList)) {
            return resultMap;
        }
        // 1.2校验分类id都是三级分类id
        ...
        if (!NumberUtils.equals(thirdCateNum, Long.valueOf(thirdCateIdList.size()))) {
        ...
        for (Long thirdCateId : thirdCateIdList) {
            ...
            resultMap.put(thirdCateId, String.format("%s/%s/%s",
    ...
    private void checkSameName(Long parentId, String name, Long currentId) {
        ...
        List<Category> categories = this.baseMapper.selectList(queryWrapper);
        // 3.新增情况下，有同名的分类
        if (currentId == null && CollectionUtils.isNotEmpty(categories)) {
            throw new BizIllegalException(CourseErrorInfo.Msg.CATEGORY_SAME_NAME);
        }
        // 4.更新情况下出现同名，需要判断是否是当前分类的名称
        if (CollectionUtils.isNotEmpty(categories) && categories.get(0).getId() != currentId.longValue()) {
            throw new BizIllegalException(CourseErrorInfo.Msg.CATEGORY_SAME_NAME);
        }
    }
    ...
    private boolean filter(CategoryVO categoryVO, CategoryListDTO categoryListDTO) {
        ...
        if (!pass && CollectionUtils.isEmpty(categoryVO.getChildren())) { // 告诉上一级没通过
            return false;
        }
        // 遍历子分类是否符合条件
        ...
        return pass || CollectionUtils.isNotEmpty(categoryVO.getChildren());
    }
    ...
    private void filter(List<SimpleCategoryVO> simpleCategoryVOS) {
        // 1.判空
        if (CollectionUtils.isEmpty(simpleCategoryVOS)) {
            return;
        }
        // 2.遍历分类列表
        for (int count = simpleCategoryVOS.size() - 1; count >= 0; count--) {
            ...
            if (CollectionUtils.isEmpty(simpleCategoryVO.getChildren())) {
