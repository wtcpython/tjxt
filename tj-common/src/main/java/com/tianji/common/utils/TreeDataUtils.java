...
public class TreeDataUtils {
    ...
    public static <T> Object ergodicTreeCalculate(List<T> data, CalculateDataProcessor<T> calculateDataProcessor) {
        ...
        for (T t : data) {
            ...
            Object currentAllData = calculateDataProcessor.calculate(tData,
                    ergodicTreeCalculate(childData, calculateDataProcessor));
    ...
    public static <T, R> void parseTreeToList(Object parentKey, List<R> originData,
        ...
        if (CollectionUtils.isNotEmpty(originData)) {
            for (R data : originData) {
                ...
                parseTreeToList(dataProcessor.getKey(data), dataProcessor.getChildren(data), dataProcessor, clazz,
                        convert, targetData, filter);
            }
        }
    }
     ...
     * @return 目标数据类型的树状数据
     */
    public static <T, R> List<T> parseToTree(List<R> originData, Class<T> clazz, Convert<R, T> convert,
            DataProcessor<T, R> dataProcessor) {
     ...
     */
    public static <T, R> List<T> parseToTree(List<R> originData, Class<T> clazz, Convert<R, T> convert,
            DataProcessor<T, R> dataProcessor, Filter<R> filter) {
        // 1.原始数据为空，返回一个空列表
