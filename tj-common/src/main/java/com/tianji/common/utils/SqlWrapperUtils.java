...
public class SqlWrapperUtils {
    ...
    public static String getSqlCoumns(Class<?> clazz, String prefix) {
        ...
        for (Field field : fields) {
            ...
            if (ReflectUtils.getMethod(clazz, "get" + StringUtils.capitalize(fieldName)) != null) {
                buffer.append(prefix)
                        .append(((PropertyNamingStrategies.SnakeCaseStrategy) PropertyNamingStrategies.SNAKE_CASE)
                                .translate(fieldName))
                        .append(",");
            }
        }
     ...
     * @param <R>           查询实体类型
     */
    public static <T, R> void suppleQueryWrapper(QueryWrapper<?> queryWrapper, String prefix, R queryDTO,
            Class<T> targetClasszz) {
        // 将查询dto转换成目标对象
        ...
        if (ArrayUtils.isNotEmpty(fields)) {
            ...
            for (Field field : fields) {
                ...
                if (value != null && ReflectUtils.getMethod(targetClasszz,
                        "get" + StringUtils.capitalize(field.getName())) != null) {
                    queryWrapper.eq(prefix + "."
