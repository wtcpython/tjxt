...
public class WxConfiguration {
    ...
    public CertificatesManager certificatesManager(WxPayProperties properties)
        ...
        // 1.加载私钥
        PrivateKey privateKey = readPemPrivateKey(properties.getPrivateKey());
        // 2.签名工具
        PrivateKeySigner privateKeySigner = new PrivateKeySigner(properties.getMchSerialNo(), privateKey);
        WechatPay2Credentials wechatPay2Credentials = new WechatPay2Credentials(properties.getMchId(),
                privateKeySigner);
        // 3.平台证书管理器
        CertificatesManager certificatesManager = CertificatesManager.getInstance();
        certificatesManager.putMerchant(
                properties.getMchId(), wechatPay2Credentials,
    ...
    public CloseableHttpClient closeableHttpClient(WxPayProperties properties, CertificatesManager certificatesManager)
            throws NotFoundException, GeneralSecurityException {
        // 1.加载私钥
        PrivateKey privateKey = readPemPrivateKey(properties.getPrivateKey());
        // 2.初始化
    ...
    private static PrivateKey readPemPrivateKey(String pem) throws GeneralSecurityException {
        String base64 = pem.replaceAll("-----BEGIN PRIVATE KEY-----", "").replaceAll("-----END PRIVATE KEY-----", "")
                .replaceAll("\\s+", "");
