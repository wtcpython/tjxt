...
public class InterestsServiceImpl extends ServiceImpl<InterestsMapper, Interests> implements IInterestsService {
    ...
    public void saveInterests(List<Long> interestedIds) {
        ...
        String ids = interestedIds == null ? ""
                : interestedIds.stream().filter(id -> id != null).map(String::valueOf).collect(Collectors.joining(","));
        ...
        interests.setInterests((interestedIds == null ? null
                : interestedIds.stream().map(String::valueOf).collect(Collectors.joining(","))));
        // 3.保存
